...
- Self-hosted runner policy: PR jobs remain GitHub-hosted; trusted extended jobs use the SHA-restricted `linux-flow-trusted` group; native repos must use self-hosted runners for required automation, with Docker, service-container, browser, or `container:` workloads routed to dedicated self-hosted capability pools.
